...
class CitySimulation {
  constructor(id, secondsPerHour = 10) {
    ...
    this.currentHourData = null; // Current hour data
  ...
  async generateNextHourData() {
    try {
      ...
      this.currentHourData = simulationData;
      ...
      console.log(`📊 Simulation ${this.id} - Hour ${this.simulationHour}: ${weather.condition}, ${trafficSummary}, Events: ${eventsSummary}`);
  ...
  async start() {
    ...
    try {
      ...
      // Generate the first hour of data
      await this.generateNextHourData();
      this.isInitialized = true;
      
      console.log(`✅ Simulation ${this.id} started and initialized`);
      
    } catch (error) {
      console.error(`❌ Failed to start simulation ${this.id}:`, error);
      this.isRunning = false;
      throw error;
    }
  }
  ...
  async advanceOneHour() {
    ...
    if (!this.isInitialized) {
      throw new Error('Simulation is not initialized yet');
    }
    
    // Generate next hour data
    const nextHourData = await this.generateNextHourData();
    return nextHourData;
  }
  
  getCurrentSnapshot() {
    if (!this.isInitialized || !this.currentHourData) {
    ...
    return {
      ...
      simulation_id: this.id,
      timestamp: this.currentHourData.timestamp,
      hour: this.simulationHour,
      is_running: this.isRunning,
      weather: {
        temperature: this.currentHourData.weather.temperature,
        humidity: this.currentHourData.weather.humidity,
        windSpeed: this.currentHourData.weather.windSpeed,
        condition: this.currentHourData.weather.condition,
        pressure: this.currentHourData.weather.pressure
      },
      events: {
        active_count: this.currentHourData.events.active_count,
        scheduled_count: this.currentHourData.events.scheduled_count,
        completed_count: this.currentHourData.events.completed_count,
        events: this.currentHourData.events.events
      },
      traffic: {
        congestion_level: this.currentHourData.traffic.congestion_level,
        average_speed: this.currentHourData.traffic.average_speed,
        total_vehicles: this.currentHourData.traffic.total_vehicles,
        peak_hour: this.currentHourData.traffic.peak_hour,
        weather_impact: this.currentHourData.traffic.weather_impact,
        events_impact: this.currentHourData.traffic.events_impact,
        datazones: this.currentHourData.traffic.datazones.map(zone => ({
  ...
  updateTimeCompression(secondsPerHour) {
    this.secondsPerHour = secondsPerHour;
    console.log(`🕐 Updated simulation ${this.id} time compression to ${secondsPerHour}s per hour`);
  }
}
...
// Traffic Simulator (keeping the same implementation as before)
