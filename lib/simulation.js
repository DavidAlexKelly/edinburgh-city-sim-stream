...
class CitySimulation {
  ...
  async generateNextHourData() {
    if (this.isGenerating) {
      console.log(`⚠️ Generation already in progress for simulation ${this.id}`);
      return;
    }
  ...
  async start() {
    ...
    try {
      ...
      console.log(`✅ Simulation ${this.id} started with first hour pre-generated`);
  ...
  async getDataAndAdvance() {
    ...
    this.generateNextHourData().catch(error => {
      console.error(`❌ Background generation failed for simulation ${this.id}:`, error);
    });
